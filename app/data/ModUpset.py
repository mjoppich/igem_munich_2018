...
class UpSet:
    ...
    def __init__(self, data, orientation='horizontal', sort_by='degree',
                 ...
                 show_counts='', sort_sets_by='deprecated', set2color=None):
    ...
    def make_grid(self, fig=None):
        ...
        totals_names = self.makeIndexNames(self.totals.index.values)
        totals_names = [x + "++++++" for x in totals_names]

        # Determine text size to determine figure size / spacing
        r = get_renderer(fig)
        t = fig.text(0, 0, '\n'.join(totals_names))
        textw = t.get_window_extent(renderer=r).width + 50
    ...
    def makeIndexNames(self, names):

        innames = [x for x in names]
            ...
            for x in innames:
                if x.startswith("/") and len(x) > 40:
                    ...
                    for xidx in range(len(x)):
                        if xidx > 2 and len(x[xidx]) > 5 and xidx < len(x)-2:
                            elem = x[xidx]
                            x[xidx] = elem[0:2] + "..." + elem[-3:]
    ...
    def plot_matrix(self, ax):
        ...
        tick_axis.set_ticklabels([x for x in data.index.names], rotation=0 if self._horizontal else -90)
        ticklabelTexts = self.makeIndexNames([x.get_text() for x in tick_axis.get_ticklabels()])

        newlabels = []
        for tidx, tlabel in enumerate(tick_axis.get_ticklabels()):
            if tlabel.get_text().upper() == "UNALIGNED":
                tlabel.set_color('orange')
            else:
                tlabel.set_color(self._set2color.get(tlabel.get_text(), self._facecolor))

            tlabel.set_text(ticklabelTexts[tidx])

            newlabels.append(tlabel)

        tick_axis.set_ticklabels(newlabels)
